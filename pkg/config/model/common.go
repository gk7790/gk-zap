...
func (c *QUICOptions) Complete() {
	c.KeepalivePeriod = value.EmptyOr(c.KeepalivePeriod, 10)
	c.MaxIdleTimeout = value.EmptyOr(c.MaxIdleTimeout, 30)
	c.MaxIncomingStreams = value.EmptyOr(c.MaxIncomingStreams, 100000)
}
...
func (c *WebServerConfig) Complete() {
	c.Addr = value.EmptyOr(c.Addr, "127.0.0.1")
}
...
func (c *LogConfig) Complete() {
	c.To = value.EmptyOr(c.To, "console")
	c.Level = value.EmptyOr(c.Level, "info")
	c.MaxDays = value.EmptyOr(c.MaxDays, 3)
}
