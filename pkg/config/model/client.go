...
func (c *ClientCommonConfig) Complete() error {
	c.ServerAddr = value.EmptyOr(c.ServerAddr, "0.0.0.0")
	c.ServerPort = value.EmptyOr(c.ServerPort, 7000)
	c.LoginFailExit = value.EmptyOr(c.LoginFailExit, lo.ToPtr(true))
	c.NatHoleSTUNServer = value.EmptyOr(c.NatHoleSTUNServer, "stun.easyvoip.com:3478")
...
func (c *ClientTransportConfig) Complete() {
	c.Protocol = value.EmptyOr(c.Protocol, "tcp")
	c.DialServerTimeout = value.EmptyOr(c.DialServerTimeout, 10)
	c.DialServerKeepAlive = value.EmptyOr(c.DialServerKeepAlive, 7200)
	c.ProxyURL = value.EmptyOr(c.ProxyURL, os.Getenv("http_proxy"))
	c.PoolCount = value.EmptyOr(c.PoolCount, 1)
	...
	if lo.FromPtr(c.TCPMux) {
		...
		c.HeartbeatInterval = value.EmptyOr(c.HeartbeatInterval, -1)
		c.HeartbeatTimeout = value.EmptyOr(c.HeartbeatTimeout, -1)
	} else {
		c.HeartbeatInterval = value.EmptyOr(c.HeartbeatInterval, 30)
...
func (c *TLSClientConfig) Complete() {
	c.Enable = value.EmptyOr(c.Enable, lo.ToPtr(true))
	c.DisableCustomTLSFirstByte = value.EmptyOr(c.DisableCustomTLSFirstByte, lo.ToPtr(true))
}
